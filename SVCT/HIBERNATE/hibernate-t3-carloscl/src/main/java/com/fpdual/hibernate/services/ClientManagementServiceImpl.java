...
public class ClientManagementServiceImpl implements ClientManagementServiceI {
	...
	public List<Client> searchByName(String firstName) {
		...
		if (firstName != null) {
			...
			// Obtención de clientes por nombre.
			clientsList = clientDao.searchByName(firstName);

			// Log de la operación.
			Utils.log(Utils.INFO, "Clientes encontrados por nombre: " + firstName);
		} else {
			// Log de error si el nombre del cliente es nulo.
			Utils.log(Utils.ERROR, "Nombre de cliente nulo.");
		}
